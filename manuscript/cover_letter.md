...
2. **The latency difference is small where it lands.** Confirming a class takes five frames, so the 8.3 ms mean difference accumulates to 41.5 ms, against an end-to-end alert path of under 2.5 s. Stating the cost in the units of the deployed path, rather than as a percentage of per-frame inference time, changes what the 45.6 % figure means for a practitioner.
...
One field result runs against the bench measurements and we report it as such. The profiling runs peaked at 68.3 °C against an 82 °C throttling threshold, but the deployed session reached 77.7 °C within three duty cycles at 94.1 % processor utilisation and was still climbing when it ended. A 3.71 °C difference between conditions is of the same order as the 4.3 °C the deployed session had left, which is what would make it operationally significant — and the field session was run in one configuration only, so whether it transfers is untested. We state the conditional rather than the conclusion.
...
**Reproducibility.** All per-frame telemetry, per-cycle event markers and the site-collected greenhouse image dataset are deposited at https://doi.org/10.5281/zenodo.22857312 under CC BY 4.0, and the analysis code, container definitions and edge application are at https://github.com/xiaolin200206/edge-container-deployment-measurement. Every table and figure in the manuscript and its supplement is regenerated from the deposited logs by a single released script; no reported quantity exists outside that chain.

# CELL 2 -- 11-model REAL_PLUS_PROXY training comparison (run CELL 1 first and
# confirm the image counts look right before running this)
# ============================================================
...
DATASET_MODE = "REAL_PLUS_PROXY"
# Results are written straight back to persistent storage, so they survive
# session disconnects / sleep cycles.
...
class LeafDataset(Dataset):
    """
    REAL_ONLY configuration (3 classes: Background/Healthy/Disease).
    The Disease class uses Real_disease only; Proxy_disease is excluded
    ...
    def __init__(self, root_dir, transform=None, mode="REAL_ONLY"):
        ...
        if mode == "REAL_ONLY":
            ...
            exclude_top_dirs = {'Proxy_disease'}
        elif mode == "REAL_PLUS_PROXY":
            class_name_map = {
                'Background': 'Background',
                'Basil_healthy': 'Healthy',
                'Real_disease': 'Disease',
                'Proxy_disease': 'Disease',
            }
        ...
        images_by_class = {}
        for top_name in sorted(os.listdir(root_dir)):
            top_path = os.path.join(root_dir, top_name)
            if not os.path.isdir(top_path) or top_name in self.SKIP_DIRS:
                continue
            if top_name in exclude_top_dirs:
                print(f"Skipping {top_name} (excluded under {mode} config)")
                continue
            if top_name not in class_name_map:
                print(f"Unrecognized folder, skipping: {top_name}")
                continue

            class_label = class_name_map[top_name]
            img_paths = collect_images_recursive(top_path)
            if len(img_paths) == 0:
                print(f"Skipping empty folder: {top_name}")
                continue

            images_by_class.setdefault(class_label, []).extend(img_paths)
            print(f"   {top_name} -> {class_label}: {len(img_paths)} images")
        ...
        for class_label in ['Background', 'Healthy', 'Disease']:
            ...
            for img_path in images_by_class[class_label]:
...
final_csv = os.path.join(OUTPUT_PATH, 'baseline_comparison_REAL_PLUS_PROXY_final.csv')

...
class BasilClassifier:
    def __init__(self):
        ...
        if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
            print("警告: 未设置 TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID，告警功能将被禁用。")
    ...
    def preprocess(self, frame):
        """frame: HxWx3, Picamera2是RGB顺序，USB是BGR。返回(1,3,224,224)的标准化tensor。"""
    ...
    def update_temporal_smoothing(self, predicted_class, confidence):
        """对最近N帧做多数投票。低置信度的帧记为'unconfirmed'，不计入投票。"""
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # 分类任务没有bounding box，直接在整张图上叠加文字
                vis = frame.copy()
                ...
                # 想看画面就用 `DISPLAY=:0 python3 basil_classifier.py` 跑，会显示在Pi的HDMI输出上
